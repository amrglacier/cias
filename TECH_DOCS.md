...
> **版本**: V1.4.2 | **最后更新**: 2026-07-23 | **文档面向**: 接手开发的工程师
...
- 使用贝叶斯平滑算法处理对手强度调整
- 通过纯数学公式计算预测结果（主推 + 备选）
- 每 15 分钟采集赔率快照，检测 Sharp/Steam 信号
- 赛后自动复盘归因，持续迭代因子权重
- 移动优先的响应式 Web 界面
...
| `betting_window_config` | 购彩时间窗口配置 | start=2h, end=15min, 5 大联赛 |
...
| Phase 5 | 终盘锁定 | 赛前 N 分钟（可配置，默认 15min） | `FINAL` | `orchestrator.ts:runPhase5_FinalPublish()` |
...
| `*/15 * * * *` | 每 15 分钟 | 赔率快照采集 + 信号检测 | `runScheduledOddsCapture()` |
...
runScheduledFundamentalsAndMonitoring():
  ...
  4. 遍历即将开始的比赛：
     a. 若在赛前窗口内（距开球 <= start_hours）且未采集过基本面：
        → 执行 Phase 1 (T0) + Phase 2 (Initial) + Phase 3 (Cross-Discussion)
     b. 若在截止窗口内（距开球 <= end_minutes）且未锁定：
...
curl -X POST https://cias-frontend.pages.dev/api/config/betting-window \
  ...
  -d '{"start_hours_before_kickoff": 3, "end_minutes_before_kickoff": 30, "league_ids": [39,140,135,78,61], "season": "2025"}'
```
...
| The Odds API | 500 次/月 | 每 15 分钟采集，约 500-1000 次/月 |
